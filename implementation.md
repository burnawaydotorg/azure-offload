...
- CDN handles WebP conversion on-the-fly (no WebP storage needed)
...
**Note:** WebP variants are generated by the CDN on-the-fly, not stored on Azure.
...
Note: CDN handles WebP conversion on-the-fly (no storage needed)
```
...
**Step 2: Upload Optimized Images (After ShortPixel Completes)**
```php
...
function azure_handle_shortpixel_optimized( $post_id, $optimization_data ) {
    ...
    // Note: CDN handles WebP conversion, we don't upload WebP files
...
- **CDN WebP Conversion**: CDN handles WebP on-the-fly, no need to upload WebP files to Azure
...
- As a user, WebP variants are generated by the CDN on-the-fly (no storage overhead)
...
- [ ] CDN WebP conversion → Verify CDN generates WebP on-the-fly (no WebP files on Azure)
